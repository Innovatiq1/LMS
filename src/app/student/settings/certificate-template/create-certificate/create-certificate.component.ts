...
resizeElement(event: MouseEvent) {
  if (this.isResizing && this.resizingElementIndex !== null) {
    ...
    this.elements[this.resizingElementIndex].width = this.resizingStartWidth + dx;
    this.elements[this.resizingElementIndex].height = this.resizingStartHeight + dy;
  }
}
  ...
  updateElementContent(event: Event, element: any) {
    ...
    if (sel && sel.rangeCount > 0) {
      ...
      if (textNode) {
        ...
        range.setStart(textNode, Math.min(cursorPos.start, textNode.textContent?.length || 0));
        range.setEnd(textNode, Math.min(cursorPos.end, textNode.textContent?.length || 0));
